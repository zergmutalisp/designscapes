...
- Work completed:
  - Removed the Selected demos / 02 label and Browse work link, then converted the header from an obsolete three-column grid to a single-item flex layout.
  - Reworded the Horizon Pier description to “A fictional pier destination rendered as an editorial advertisement.” and changed the footer link to “GitHub” without an arrow.
  - Removed unused navigation CSS, rendered the hero at desktop and phone sizes, and added browser regressions for the revised header, description, and footer text.
